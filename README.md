> **Evaluation experiment — `experiment/dido-sparse-profile`**: [final report](docs/implementation/dido-sparse-profile/REPORT.md), [verification](docs/implementation/dido-sparse-profile/FINAL-VERIFICATION.json), [implementation](docs/implementation/dido-sparse-profile/ONLINE.md), [data sources](docs/implementation/dido-sparse-profile/SOURCES.md), [workflow](docs/implementation/dido-sparse-profile/WORKFLOW.md). The bounded study is complete and stopped at **12 real episode attempts**. Development: Dense 3/3, candidate 3/3; additional fixed validation: Dense 2/3, candidate 3/3. These tiny cohorts do **not** certify the 5-percentage-point SR margin. Same-input warm prediction reaches 2.130× through visual feature reuse, while new selectors worsen action proxies versus inherited uniform. CPU/CUDA, raw replay and the real adapter pass; task resources are released. Historical [paper-evaluation results](docs/action-eval/README.md) remain separate. Upstream attribution is preserved; execution remains server-only with unchanged dependencies.
