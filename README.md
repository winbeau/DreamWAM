> **Evaluation fork — `main`**: [paper-evaluation status](docs/action-eval/README.md), [agent rules](AGENTS.md), [earlier consolidated results](docs/action-eval/sparse-results-paused-20260920.md). The user resumed GPU 4–7 work for [every-step 10% visual tokens without cross-step visual caching](docs/action-eval/fresh-visual-tokens-20260920.md). Its first shared-load measurement is 1.191× versus strengthened Dense; quality remains unverified. The older cached candidate's 1.872× is separate evidence. Screening remains 50 episodes per candidate, with full tests deferred until method selection. Upstream instructions below are preserved for reference; execution remains server-only and dependency versions are preserved.
