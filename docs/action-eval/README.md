...
Current continuation: [action-guided visual token refresh](visual-token-cache-single-factor-20260919.md) measures 1.657× full-request speedup, with a separate official pilot in progress. Token selection adds little speed beyond temporal reuse; its quality benefit is unproven. [Temporal-cache experiments](visual-cache-single-factor-20260919.md) measure 1.655× for two visual refreshes and 1.803× for one, retaining all ten action steps. The two-refresh candidate and matched Dense completed all 15 paired pilot episodes successfully. Full 500-episode Spatial runs are progressing through bounded native-renderer recovery, with incomplete SR withheld. [FFN-only and action-guided neuron negative results](ffn-cache-single-factor-20260919.md) are preserved. Older pause and GPU-7-only notes below are historical; the current user allows idle or lightly occupied GPUs.
