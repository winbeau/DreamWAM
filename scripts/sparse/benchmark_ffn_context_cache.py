...
def main():
    ...
    parser.add_argument("--cache-kind", choices=("tokens", "neurons", "guided_neurons", "visual_steps", "visual_tokens", "guided_visual_tokens"), default="tokens")
    ...
    manifest = dict(status="RUNNING", start_utc=datetime.now(timezone.utc).isoformat(),
                    ...
                    source_sha256={name: sha256(name) for name in (
                        "dreamwam/sparse/ffn_context_cache.py", "dreamwam/sparse/ffn_neuron_cache.py", "dreamwam/sparse/action_guided_neuron_cache.py", "dreamwam/sparse/visual_step_cache.py", "dreamwam/sparse/visual_token_cache.py", "dreamwam/sparse/action_guided_visual_token_cache.py", "scripts/sparse/benchmark_ffn_context_cache.py",
                    ...
                    factor=dict(name=("current-action guidance for partial visual token refresh" if args.cache_kind == "guided_visual_tokens" else
                                      "whole-transformer visual token refresh budget at fixed cadence" if args.cache_kind == "visual_tokens" else
                                ...
                                refresh_every=args.refresh_every if args.cache_kind in ("visual_steps", "visual_tokens", "guided_visual_tokens") else None,
                                reference=("per-row latest refreshed transformer input/KV/output" if args.cache_kind in ("visual_tokens", "guided_visual_tokens") else
                                           "per-row latest computed input/output" if args.cache_kind == "tokens" else "dense current-request group anchor"),
                                first_step="dense for every layer", action_guidance=args.cache_kind in ("guided_neurons", "guided_visual_tokens"),
                                guidance_weight=args.guidance_weight if args.cache_kind in ("guided_neurons", "guided_visual_tokens") else None,
                                additional_control=("unguided token selection with identical ratio/cadence, plus all-token temporal baseline" if args.cache_kind == "guided_visual_tokens" else
                                                    "all-token refresh at the identical temporal cadence" if args.cache_kind == "visual_tokens" else
    ...
    (out / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n")
    candidate = {"tokens": "context_cache", "neurons": "neuron_cache", "guided_neurons": "guided_neuron_cache", "visual_steps": "visual_step_cache", "visual_tokens": "visual_token_cache", "guided_visual_tokens": "guided_visual_token_cache"}[args.cache_kind]
    ...
    elif args.cache_kind == "guided_visual_tokens":
        caches = {
            "matched_dense": ActionGuidedVisualTokenCache(policy.model, keep_ratio=1, refresh_every=1,
                                                        guidance_weight=args.guidance_weight),
            "temporal_only": VisualTokenRefreshCache(policy.model, keep_ratio=1, refresh_every=args.refresh_every),
            "unguided_visual_tokens": VisualTokenRefreshCache(policy.model, keep_ratio=args.keep_ratio, refresh_every=args.refresh_every),
            candidate: ActionGuidedVisualTokenCache(policy.model, keep_ratio=args.keep_ratio, refresh_every=args.refresh_every,
                                                   guidance_weight=args.guidance_weight),
        }
    ...
    if args.cache_kind in ("visual_tokens", "guided_visual_tokens"):
    ...
    result = dict(status="MEASURED", end_utc=datetime.now(timezone.utc).isoformat(),
                  ...
                  sr=None, limitations=["No closed-loop SR; action error is a diagnostic, not a success criterion.",
                                       ("Temporal cadence is fixed; token refresh and its action-guidance control are measured separately. No neuron cache, key pruning, or public operator changes."
                                        if args.cache_kind in ("visual_tokens", "guided_visual_tokens") else
    ...
    if args.cache_kind == "guided_visual_tokens":
        result["speedup_vs_unguided_mean"] = statistics.fmean(samples["unguided_visual_tokens"]) / statistics.fmean(samples[candidate])
