...
def eligible_devices(csv_text, authorized=(3, 4, 5), *, share_gpu5=False):
    ...
    for row in rows:
        ...
        index, uuid, name, used, total, *remaining = (v.strip() for v in row)
        util = remaining[-1]
        index, used, total, util = int(index), int(used), int(total), int(util)
        # Legacy six-column fixtures omit driver-reserved memory. Live queries
        # always include actual memory.free, rather than total minus used.
        free = int(remaining[0]) if len(remaining) == 2 else total - used
        memory_ok = used <= 1024 or (share_gpu5 and index == 5)
        if index in authorized and "H100" in name and memory_ok and util <= 10 and free >= 50000:
            devices.append(dict(index=index, uuid=uuid, used_mib=used, free_mib=free, total_mib=total, utilization=util))
    return devices


def admit_profile(uuid, *, authorized=(3, 4, 5), share_gpu5=False):
    command = ["nvidia-smi", "--query-gpu=index,uuid,name,memory.used,memory.total,memory.free,utilization.gpu",
    ...
    if selected is None:
        raise RuntimeError(f"selected GPU is not currently admitted: need >=50000 MiB free, <=10% utilization, "
                           f"and authorized placement (GPU 5 sharing={share_gpu5}); inventory:\n{text}")
