#!/usr/bin/env bash
# Prepared CPU-rendered 10-pair run; release owned holds and launch in one shell.
set -euo pipefail
[[ $# -eq 2 ]] || { echo 'usage: run-dido-tenpairs.sh FROZEN_EVAL_ROOT OUTPUT_ROOT' >&2; exit 2; }
DIDO_EVAL_TREE=$1
DIDO_OUT=$2
...
"$MODEL_PYTHON" - "$DIDO_HOLD" "$DIDO_HOLD_STATE" <<'PY'
import importlib.util,json,sys
from pathlib import Path
spec=importlib.util.spec_from_file_location('hold',sys.argv[1])
module=importlib.util.module_from_spec(spec)
spec.loader.exec_module(module)
workers=json.loads(Path(sys.argv[2]).read_text())['workers']
...
dido_rehold() {
  ...
  for attempt in {1..30}; do
    if "$MODEL_PYTHON" "$DIDO_HOLD" start \
      --state "$DREAMWAM_SR_ROOT/gpu-hold/h100-tenpairs-after-gpu${gpu}.json" \
...
dido_lane() (
  ...
  "$EVAL_PYTHON" "$DIDO_CONTROLLER/scripts/sparse/run_fresh_token_pair.py" \
    ...
    --dense-config "$ACTION_EVAL_ROOT/configs/experiments/dreamwam-dido-dense-h100-tenpairs-${lane}.yaml" \
    --sparse-config "$ACTION_EVAL_ROOT/configs/experiments/dreamwam-dido-native-va56-h100-tenpairs-${lane}.yaml" \
    --planned-episodes 5 --first-arm "$first" --authorized-gpus 3 4 5 \
    --render-backend osmesa --episode-ledger "$DIDO_LEDGER" \
...
"$MODEL_PYTHON" "$DIDO_HOLD" stop --state "$DIDO_HOLD_STATE" > "$DIDO_OUT/hold-release.json"
